...
import { PUT_EDITAR } from '../services/api/tweet'
...
const FormularioTweetEditar = ({ tweet }) => {
  // ESTADOS
  ...
  // USE APP
  const { user, editarTweet, socket } = useApp()
  ...
  useEffect(() => {
    if (tweet) {
      setCampos({
        ...
        menciones: tweet.menciones,
        foto: tweet.foto,
        eliminado: false
      })

      setContenido([...HashtagsToJsx({ hashtags: tweet.hashtags }), ...MencionesToJsx({ menciones: tweet.menciones })])

      const currentLength = tweet.descripcion.length
  ...
  const handleChange = (e) => {
    ...
    if (value.length <= MAX_CHARS + 1) {
      ...
      const mencionesNotNull = mencionesMapeadas.filter((m) => m !== null)
  ...
  const handleEliminarFoto = () => {
    setCampos({ ...campos, foto: {}, eliminado: true })
  }

  const handleSubmit = async (e) => {
    e.preventDefault()
    const { foto, descripcion, hashtags, menciones, eliminado } = campos
    ...
    try {
      ...
      const { data } = await PUT_EDITAR({ id: tweet._id, eliminado, descripcion, foto, hashtags, menciones: mencionesIds })
      setCampos(DEFAULT_CAMPOS)
      toast.success(data.message, { icon: '🖋' })
      // SINCRONIZAR
      editarTweet(data.tweet)
      // NOTIFICACIONES
...
export default FormularioTweetEditar
