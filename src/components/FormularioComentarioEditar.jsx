...
import { PUT_COMENTARIO } from '../services/api/comentario'
...
const FormularioComentarioEditar = ({ comentario, setComentarios, callback = () => {} }) => {
  // ESTADOS
  ...
  // USE APP
  const { user, socket, loadingUser } = useApp()
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    const { foto, texto, eliminado, menciones } = campos
    ...
    try {
      ...
      const { data } = await PUT_COMENTARIO({ tweetId, comentarioId: comentario._id, eliminado, texto, foto, mencionesIds })
      setCampos(DEFAULT_CAMPOS)

      toast.success(data.message, { icon: '📝' })
      // SINCRONIZAR
      setComentarios((prev) => {
        prev = prev.map((c) => (c._id === comentario._id ? data.comentario : c))
        return prev
      })
      ...
      <div className='flex'>
        ...
        <div className='box-compose'>
          <form onSubmit={handleSubmit} className='flex'>
            <textarea ref={inputRef} name='texto' id='texto' autoFocus value={campos.texto} onChange={handleChange} cols='30' rows='10' placeholder='Escribir comentario...'></textarea>
...
export default FormularioComentarioEditar
