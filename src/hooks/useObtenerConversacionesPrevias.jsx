...
import { GET_CONVERSACIONES_PREVIEW } from '../services/api/mensaje'
import useApp from './useApp'

const useObtenerConversacionesPrevias = () => {
  // ESTADOS
  ...
  const obtenerMensajes = async () => {
    ...
    try {
      const { data } = await GET_CONVERSACIONES_PREVIEW({ page, limit: 20 })
    ...
    if (user._id) {
      ...
      socket.on('/usuario/conversaciones/obtener', (conversacionEmitida) => {
        ...
        if (!existe) {
          ...
          const conversacionesActualizadas = [conversacionEmitida, ...conversaciones.filter((c) => c._id !== conversacionEmitida._id)]
...
export default useObtenerConversacionesPrevias
